#!/bin/bash

#Java Setup
setup_java(){
  cd ${BUILD_DIR}
  # Make sure we can see uname
  export PATH=$PATH:/bin:/usr/bin

  #unpack Java - we support Mac OS 64bit and Linux 64bit otherwise we require JAVA_HOME to point to JDK
  mkdir java
  tar zxvf openjdk/OpenJDK11U-jdk_x64_linux_hotspot_11.0.11_9.tar.gz -C java --strip-components=4
  export JAVA_HOME=${BUILD_DIR}/java

  #setup Java path
  export PATH=$JAVA_HOME/bin:$PATH
}

#Maven Setup
setup_maven(){
  cd ${BUILD_DIR}
  tar zxvf maven/apache-maven-3.6.3-bin.tar.gz -C maven --strip-components=1
  export M2_HOME=${BUILD_DIR}/maven
  export PATH=$M2_HOME/bin:$PATH
}

#Cleanup Java files from BUILD_DIR
cleanup_java(){
  cd ${BUILD_DIR}
  rm -rf java
  rm -rf openjdk
}

#Cleanup Maven files from BUILD_DIR
cleanup_maven(){
  cd ${BUILD_DIR}
  rm -rf maven
}

