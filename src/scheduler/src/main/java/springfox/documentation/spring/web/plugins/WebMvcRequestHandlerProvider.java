...
public class WebMvcRequestHandlerProvider implements RequestHandlerProvider {
  ...
  public WebMvcRequestHandlerProvider(
    ...
    this.contextPath = servletContext.map(ServletContext::getContextPath).orElse(Paths.ROOT);
  }
  ...
  private Function<
          RequestMappingInfoHandlerMapping, Iterable<Map.Entry<RequestMappingInfo, HandlerMethod>>>
      toMappingEntries() {
    return input -> input.getHandlerMethods().entrySet();
  }

  private Function<Map.Entry<RequestMappingInfo, HandlerMethod>, RequestHandler>
      toRequestHandler() {
    return input ->
        new WebMvcRequestHandler(contextPath, methodResolver, input.getKey(), input.getValue());
  }
}
