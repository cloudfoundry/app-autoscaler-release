...
public class HealthEndpointWithBasicAuthTest {
    ...
    @Autowired
    private HealthServerConfiguration HealthServerConfig;
    ...
    public void givenIncorrectCredentialsShouldReturn401() {
        ResponseEntity<String> response =
                this.restTemplate.withBasicAuth("bad", "auth").getForEntity(prometheusMetricsUrl(),
                        String.class);
        assertThat(response.getStatusCode().value()).describedAs("Http status code should be Unauthorized").
                isEqualTo(401);
    }

    @Test
    public void givenNoCredentialsShouldReturn401() {
        ResponseEntity<String> response = this.restTemplate.getForEntity(prometheusMetricsUrl(), String.class);
    ...
    public void givenCorrectPasswordAndWrongUsernameFailsWith401() {
        ResponseEntity<String> response =
                this.restTemplate.withBasicAuth("bad", "someHash").getForEntity(prometheusMetricsUrl(),
                        String.class);
        assertThat(response.getStatusCode().value()).isEqualTo(401);
    }
    ...
    public void givenIncorrectCredentialsShouldLivenessReturn401() {
        ResponseEntity<String> response =
                this.restTemplate.withBasicAuth("bad", "auth").getForEntity(livenessUrl(),
                        String.class);
        assertThat(response.getStatusCode().value()).
                describedAs("Http status code should be Unauthorized").
                isEqualTo(401);
