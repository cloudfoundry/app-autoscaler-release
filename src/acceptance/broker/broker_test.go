...
var _ = Describe("AutoScaler Service Broker", func() {
	...
	BeforeEach(func() {
		...
		createApp := cf.Cf("push", appName, "--no-start", "-b", cfg.NodejsBuildpackName, "-m", fmt.Sprintf("%dM", cfg.NodeMemoryLimit), "-p", config.NODE_APP).Wait(cfg.DefaultTimeoutDuration())
		Expect(createApp).To(Exit(0), "failed creating app")

		mapRouteToApp := cf.Cf("map-route", appName, cfg.AppsDomain)
