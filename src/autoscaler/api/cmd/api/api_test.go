...
var _ = Describe("Api", func() {
	...
	Describe("when Health server is ready to serve RESTful API with basic Auth", func() {
		...
		Context("when /health/readiness is called", func() {
			It("should return 200 and correct checkers", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(body).To(MatchJSON(`{ "overall_status": "UP", 
"checks": [ 
 ...
 { "name": "cf_api", "type": "service", "status": "UP" }
]}`))
			})
