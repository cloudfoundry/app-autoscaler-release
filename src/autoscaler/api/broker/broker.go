...
func (b *Broker) Provision(ctx context.Context, instanceID string, details domain.ProvisionDetails, _ bool) (domain.ProvisionedServiceSpec, error) {
	...
	if policy != nil {
		...
		// If policyJson was invalid we would not have created the policy above. Therefore …
		policyBytes := []byte(policyJson) // … we can just use the original JSON here.
		if err != nil {
...
func (b *Broker) Bind(
	...
	if customMetricsBindingAuthScheme == &models.OAuth2BearerToken {
		// create credentials
